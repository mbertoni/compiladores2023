...
run ck@(CONST : n : c) e s = do
  printState ck e s
  run c e (Natural n : s)
run ck@(ADD : c) e (Natural n : Natural m : s) = do
  printState ck e (Natural n : Natural m : s)
  run c e (Natural (m + n) : s)
run ck@(SUB : c) e (Natural y : Natural x : s) = do
  printState ck e (Natural y : Natural x : s)
  run c e (Natural (max (x - y) 0) : s)
run ck@(CALL : c) e (v : Fun ef cf : s) = do
  printState ck e (v : Fun ef cf : s)
...
run ck@(DROP : c) (v : e) s = do
  printState ck (v : e) s
  run c e s
run ck@(SHIFT : c) e (v : s) = do
  printState ck e (v : s)
  run c (v : e) s
run ck@(IFZ : c) e s = do
  printState ck e s
  run c e s
run ck@(JUMP : c) e s = do
  printState ck e s
  failFD4 "Unimplemented Jump"
run ck@(FIX : c) e (Fun env cf : s) = do
  printState ck e (Fun env cf : s)
  run c e (Fun ef cf : s)
  where
    ef = Fun ef cf : env
...
printState c e s = do
  printFD4 (showBC c ++ " - " ++ showVal e ++ " - " ++ showVal s)
  return ()
