...
openAll gp ns (Var p v) = case v of
  ...
  Free x -> SV (gp p) x
  Global x -> SV (gp p) x
openAll gp ns (Cst p c) = SCst (gp p) c
...
openAll gp ns (Pnt p str t) = SPrint (gp p) str (openAll gp ns t)
openAll gp ns (BOp p op t u) = SBinaryOp (gp p) op (openAll gp ns t) (openAll gp ns u)
